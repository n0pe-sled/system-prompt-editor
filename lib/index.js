...
*/
const PERSONA_SECTION$1 = "deployment:persona";
/** Valid variable names: how they are written between the braces. */
const VARIABLE_NAME$1 = /^[a-z][a-z0-9_]*$/;
...
var SystemPrompt = class extends Service {
	...
	*/
	variable(name, provider) {
		if (!VARIABLE_NAME$1.test(name)) throw new Error(`invalid prompt variable name "${name}" (must match ${String(VARIABLE_NAME$1)})`);
...
new Set(MANAGED_SECTIONS.map((entry) => entry.name));
new Set([...SECTION_CATALOG, ...MANAGED_SECTIONS].map((entry) => entry.name));
...
const ORDER_BY_NAME = {
	...
	[SOURCE_SECTION]: -99,
	[WEB_SURFACE_SECTION]: -98,
	[PERSONA_SECTION]: 0,
	[PLAN_POLICY_SECTION]: 50,
	[FILE_REFERENCE_SECTION]: 99,
	[CODE_ONLY_SECTION]: 99,
	[TOOL_GUIDANCE_SECTION]: 150,
	[SDK_SECTION]: 150,
	[DELIVERABLES_SECTION]: 190,
	[CUSTOM_SECTION]: 200
};
/**
...
* @returns the band.
*/
function bandOfSection(name) {
	if (name === "harness:identity") return "identity";
	if (name === "harness:source") return "source";
	if (name === "app:web-surface") return "web-surface";
	if (name === "deployment:persona" || name === "deployment:persona-prefix" || name === "deployment:persona-suffix") return "persona";
	if (name === "plan:policy") return "plan";
	if (name === "context:file-reference") return "file-reference";
	if (name === "tools:code-only") return "code-only";
	if (name === "user:tool-guidance" || name.startsWith("tool:")) return "tool-guidance";
	if (name === "tools:sdk") return "sdk";
	if (name === "ui:deliverable-file-references") return "deliverables";
	if (name === "user:system-prompt-editor") return "custom";
	return "other";
}
/**
...
*/
function knownSectionOrder(name) {
	const order = ORDER_BY_NAME[name];
	if (order !== void 0) return order;
	if (name === "user:tool-guidance" || name.startsWith("tool:")) return 150;
}
//#endregion
...
*/
function applyOverrides(assembly, overrides, options = {}) {
	if (overrides.persona !== void 0 && overrides.persona !== "") {
		const slot = assembly.sections.find((section) => section.name === PERSONA_SECTION$1);
	...
	if (overrides.sections !== void 0) for (const [name, value] of Object.entries(overrides.sections)) {
		if (value === "") continue;
		...
		if (order !== void 0) {
			const anchor = assembly.sections.findIndex((section) => {
				...
				return known !== void 0 && known > order;
			});
			if (anchor >= 0) index = anchor;
		}
		assembly.sections.splice(index, 0, {
			name,
			text: value
		});
	}
...
function knownOrder(name, customOrder) {
	if (name === "user:system-prompt-editor") return customOrder;
...
*/
function renderPreviewPrompt(assembly) {
	return assembly.sections.map((section) => interpolatePreview(section.text, assembly.variables)).filter((text) => text.length > 0).join("\n\n");
}
/** Interpolate {@link text} leniently for the preview display. */
function interpolatePreview(text, variables) {
	let result = "";
	let last = 0;
	for (let open = text.indexOf("{{"); open >= 0; open = text.indexOf("{{", last)) {
		...
		const value = VARIABLE_NAME.test(name) && Object.hasOwn(variables, name) ? variables[name] : void 0;
		if (value === void 0) result += text.slice(last, open + group[0].length);
		else result += text.slice(last, open) + value;
...
*/
function previewVariableFallbacks(ctx) {
...
function apply(ctx, config) {
	const sectionSchema = z.object({
		...
		sections: z.dict(z.string()).default({})
	});
	const scope = ctx.settings.register(NAMESPACE, sectionSchema);
	const toolGuidanceSnapshot = /* @__PURE__ */ new Map();
	...
	ctx.on("system-prompt/assemble", async (assembly, _context, next) => {
		...
		for (const section of assembly.sections) if (section.name.startsWith("tool:")) toolGuidanceSnapshot.set(section.name, section.text);
	...
	const receiver = {
		...
		async preview(drafts) {
			...
			for (const [name, value] of Object.entries(previewVariableFallbacks(ctx))) if (assembly.variables[name] === void 0) assembly.variables[name] = value;
			if (!assembly.sections.some((section) => section.name.startsWith("tool:") || section.name === "user:tool-guidance") && toolGuidanceSnapshot.size > 0) {
				const anchor = assembly.sections.findIndex((section) => {
					const order = knownSectionOrder(section.name);
					return order !== void 0 && order > 100;
				});
				const entries = [...toolGuidanceSnapshot.entries()].map(([name, text]) => ({
					name,
					text
				}));
				if (anchor < 0) assembly.sections.push(...entries);
				else assembly.sections.splice(anchor, 0, ...entries);
			}
			const stored = scope.get();
			const effective = {
				text: stored?.text ?? "",
			...
			try {
				return {
					rendered: renderPreviewPrompt(assembly),
					sections,
					effective
				};
			} catch (error) {
...
//#endregion
export { Config, MANAGED_SECTIONS, SECTION_CATALOG, apply, applyOverrides, bandOfSection, inject, knownSectionOrder, name };
